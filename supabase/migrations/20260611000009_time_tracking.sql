-- Time tracking in Jira-style "2w 4d 6h 45m" format.
-- Canonical storage is integer MINUTES (parsed/formatted in the app using
-- Jira ratios: 1w = 5d, 1d = 8h, 1h = 60m). Replaces the old numeric *_hours.

alter table public.issues
  add column estimate_minutes integer,
  add column spent_minutes integer;

-- Backfill existing rows (hours → minutes), then drop the old columns.
update public.issues
  set estimate_minutes = round(estimate_hours * 60),
      spent_minutes = round(spent_hours * 60)
  where estimate_hours is not null or spent_hours is not null;

alter table public.issues
  drop column estimate_hours,
  drop column spent_hours;

-- Recreate the issue RPCs to read/write the new minute columns.
...
begin
  ...
  values (
    ...
    nullif(payload->>'estimate_minutes', '')::int,
    nullif(payload->>'spent_minutes', '')::int
  )
